...
public class SqlPrintUtils {
     ...
     * 如果耗时还是没解决考虑异步
     */
    public static void printSqlLog(long startTime, BoundSql boundSql, Boolean printQuery) {
        Object parameterObject = boundSql.getParameterObject();
        String sql = boundSql.getSql().trim();

        // 不打印查询sql日志
        if (sql.toLowerCase().startsWith("select") && BooleanUtils.isFalse(printQuery)) {
            return;
        }
...
//        ActiveSpan.info("SQL：[" + allSql + "]   执行耗时    [" + sqlCost + "ms]");
     ...
     * 验证当前参数是长度很大的list
     */
    private static boolean verifyBigArray(Object paramObj) {
        if (paramObj instanceof DefaultSqlSession.StrictMap) {
            ...
            for (Object value : strictMap.values()) {
                if (value instanceof Collection) {
                    Collection value1 = (Collection) value;
                    return value1.size() > 2;
                }
            }
            ...
            MapperMethod.ParamMap paramObj1 = (MapperMethod.ParamMap) paramObj;
            for (Object value : paramObj1.values()) {
                if (value instanceof Collection) {
     ...
     * 处理mybatis 参数
     */
    private static String handleMybatisMapParameter(String sql, MapperMethod.ParamMap paramMap, List<ParameterMapping> parameterMappingList) {
     ...
     * 获取完整的sql（推荐方案）
     */
    public static String getFllSql(BoundSql boundSql, Object parameterObject) {
    ...
    public static String formatParamValue(Object paramValue) {
        if (paramValue == null) {
            return "null";
        }
